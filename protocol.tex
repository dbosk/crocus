...
\begin{figure}
  \centering
  \begin{tikzpicture}[<-,item/.style={rectangle,draw}]
    \tikzset{%
      %grow'=left,%
      %level distance=5em%
    }
    \node[item] (proof) {Proof share}
      ...
      (wid) edge[bend left] node [anchor=north] {$PRF_{k_w}(\cdot)$} (pid)
      (cid) edge node [anchor=east] {$H(\cdot)$} (manifesto)
      (pid) edge node [anchor=east] {$PRF_{k_p}(\cdot)$} (cid)
      ;
