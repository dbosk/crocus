...
\begin{figure}
  ...
  \begin{tikzpicture}[%
    ...
    \node[item] (proof) {Proof share}
      ...
      (l) edge node [anchor=east] {$\in$} (proof)
      (wid) edge node [anchor=east] {$\in$} (proof)
      (wsig) edge node [anchor=east] {$\in$} (proof)
      ;
  ...
  \caption{%
    ...
    The protester's identifier \(pid\) is computed using the protester's key 
    \(k_p\).
    The witness's identifier \(wid\) is computed using the witness's key 
    \(k_w\).
