...
\begin{theorem}[\acs*{DBMF} resistance]
  ...
  \(\Adv_{\DBS}^{\text{MF}}(\adv) = \frac{1}{2^n}\), where \(m, q\) are as in 
  \cref{DB-Schnorr}.
\end{theorem}
...
\begin{proof}
  ...
  The first option yields success with probability \(\frac{1}{2}\), since the 
  choice is uniformly distributed over the two challenges.

  The second option yields a lower success probability:
  Assume that each bit reveals some information about the future bits.
  The first bit must still be guessed, this is yields a probability of \emph{at 
  most} \(\frac{1}{2}\).

  The best strategy is to guess which challenge the verifier will select.
  Thus, the success probability is \(\frac{1}{2}\) per round, or 
  \(\frac{1}{2^n}\) in total.
\end{proof}
