...
\Ac{DB} protocols were first suggested by \citet{distance-bounding} to prevent relay attacks in contactless communications in which the adversary forwards a communication between a prover and a possibly far-away verifier to authenticate. 
...
The choice of the anonymous credential system will affect the properties we require of the \ac{PPK} (\eg that it must be \iac{PKE} for discrete logarithms).