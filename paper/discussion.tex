...
or extra ballots for voters. Similarly, here, we need a
\begin{enumerate*}
\item\label{discussion-credential-map} mapping between an identity and a 
...
\item\label{discussion-one-credential} only one such credential per identity 
  (token of personhood instead of token of being a voter in a particular 
  election).
\end{enumerate*}
