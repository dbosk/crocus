...
or extra ballots for voters. Similarly, here, we need a 1) mapping
between an identity and a physical person (in the form of an anonymous
identity credential, functioning as a pseudonym for privacy
properties) and 2) only one such credential per identity (token of
personhood instead of token of being a voter in a particular
election). With the same caveat as for identities for voting for 1),
mechanisms such as collective signing~\cite{collective-signing} could
ensure 2). Again, this is not yet realized, but with the current pace
for adopting public ledgers and efforts for cross-national identity
credentials such as the EU's eIDAS, the prerequisites exist.
...
witnesses. In the case of the canonical \CROCUS with trusted
witnesses, as in any trusted third party, it simply reduces to their
trustworthiness.
