...
If we do not have \iac{MPC} base solution as suggested above, we can still 
reduce the problem.
When a verifier verifies an anonymous authentication, it is clear who the 
\ac{CA} (\ie the issuer) is.
In this case the verifier can analyze whether the issuer has any incentives for 
a Sybil attack.

This is similar to detecting bias in science.
If the authors personally benefit from the results, then they are incentivized 
to bias them.
The verifiability properties of our scheme serve the same purpose as the 
methodology section of a scientific paper: anyone is able to repeat, reproduce 
and verify the results --- even if they are in distant lands.
