...
Thus, we still have information theoretic security for \(s_{1-b}\)
although we reveal the \(l\) bits. (See \cref{l-bit-problem for an
  in-depth treatment of what we call the \(l\)-bit problem.)
...
\daniel{I should prove this for the composed protocol, I should check
  the details with Douglas.}

Besides the security against the standard attacks on distance-bounding
protocols, we also need to ensure that our modification does not leak
any information to the adversary via the aforementioned \(l\) bits. 

\subsection{The \(l\)-bit problem}
\label{l-bit-problem}

We will now systematically investigate the concern of the adversary
gaining any advantage from learning \(l\) bits from one of the two challenges.
...
The whole Tamarin code we wrote is provided in \cref{apdx:tamarin-spec}, with a short discussion of the modeling choices that we made. 
