...
counting% and the adversary model
.
...
Furthermore, we assume that each participant has a digital certificate of her 
identity signed by \iac{CA}\footnote{%
  The \ac{CA} must be \emph{logically centralized} to prevent Sybil 
  attacks~\cite{SybilAttack}.
...
During the protest, the devices might be limited by their batteries and computations they can perform and only have local connectivity to each other.
