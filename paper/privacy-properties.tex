...
\daniel{%
  ...
  There is receipt freeness for how a elector voted, but not that they voted.
...
\item\label{ProofUnlink} \emph{Proof Unlinkability (Anonymity)}: every participant's real (long-term) identity (\(P\)) must be unlinkable to their participation proofs \(\prf_{\pid, \prtst}\)
