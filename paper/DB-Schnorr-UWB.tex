...
Then we will adapt it by introducing \iac{AKE} protocol.
...
The \ac{PKI} is based on CL04~\cite{CLsignatures} signatures and is a 
simplified version of the Direct Anonymous Attestation 
protocol~\cite{DirectAnonymousAttestation} standardized by the Trusted 
Computing Group for \acp{TPM}.
...
\begin{figure*}
  ...
  \begin{tabular}{p{0.40\linewidth}cp{0.40\linewidth}}
    ...
    \multicolumn{2}{l}{%
      Static \(\hat g, \hat q, \sk_P\in \ZZ_q, \pk_P = \hat g^{\sk_P}, \sigma\)
    }
    & Static \(\hat g, \hat q\)
    \\
...
key~\(\sk_P\) for the prover with a digital certificate~\(\cert_P\) (and 
\(\cert_V\)) for the public key~\(\pk_P\) (and \(\pk_V\)).
