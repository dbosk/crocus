...
\ac{TPM}, so our approach can make \ac{DAA} \acl{DB}.

In summary, the difference is as follows.
\(\DBS\) has a shared key~\(k\) and performs \(
  \PK[\alpha][A = g^\alpha]
\) with a distance bound.
\(\DBSHW\) first runs \iac{DHKE} to establish the shared key~\(k =
\hat g^{xy}\) from \(X = \hat g^x, Y = \hat g^y\), then performs \[
  \PK[\alpha, x, \sk_P, \sigma][
    A = g^\alpha \land
    X = \hat g^x \land
...
Proving knowledge of \(x\) prevents any \ac{MITM} attack against the \ac{DHKE}.
...
\begin{figure*}
  ...
  \begin{tabular}{p{0.40\linewidth}cp{0.40\linewidth}}
    ...
    \(\hat s_1\gets \hat\rho - c_1 x \pmod q\)
    &
    &
    \\
    ...
    \(\hat s_m\gets \hat\rho - c_m x \pmod q\)
    &
    &
    \\
  ...
  \caption{%
    ...
    pre-setup phase).
    The protocol actually proves \(\PK[\alpha, x][A = g^\alpha \land
      \hat A = \hat g^x]\).
    This is ties \(x\), and consequently \(k\), to \(\alpha\).
...
We have public parameters \(q, G, g, \G, \g, e\); where \(e\colon G\times G\to 
\G\) is a bilinear map, \(g\in G\) and \(\g\in \G\) are generators of prime 
order \(q\) (hence \(e(g, g) = \g\)).

We let the \acg{CA} signing key be \(\sk = (x, y, z)\rgets \ZZ_q^3\) and the 
public key be \(\pk = (q, G, \G, g, \g, e, X, Y, Z)\), where \(X = g^x, Y = 
g^y, Z = g^z\).

The prover chooses \(\sk_P\rgets \ZZ_q\) and computes a commitment \(M = 
g^{\sk_P} Z^r\), where \(r\rgets \ZZ_q\).
Now the prover convinces the \ac{CA} that he knows the key by running the 
following \ac{ZKPK} protocol: \(
  \PK[\sk_P, r][M = g^{\sk_P} Z^r].
\)

Now the \ac{CA} computes the signature~\(\sigma = (a, A, b, B, c)\), where
\begin{itemize}
  \item \(a\gets g^\alpha\), where \(\alpha\rgets \ZZ_q\),
  \item \(A\gets g^z, b\gets a^y, B\gets A^y\),
  \item \(c\gets a^x M^{\alpha x y}\).
\end{itemize}
...
The prover chooses \(x\rgets \ZZ_q\), the verifier chooses \(y\rgets \ZZ_q\) 
and they exchange \(X = \hat g^x, Y = \hat g^y\) and both compute
\(k = \hat g^{xy}\), for some generator \(\hat g\) in \iac{DH} group of prime 
order \(q\).

We will see later that if the adversary attempted \iac{MITM} attack, our 
\(\DBSHW\) protocol will detect this, because the adversary will fail 
\iac{ZKPK}.

\paragraph*{Setup}

The setup phase is similar to \(\DBS\) (\cref{SchnorrFigure}).
They use the agreed upon key~\(k = g^{xy}\) as the key for \(\UWBPR\).
\(\DBS\) performs \(
  \PK[\alpha][A = g^\alpha]
\) (with a distance bound) whereas \(\DBSHW\) must perform \(
  \PK[\alpha, x][\hat A = g^\alpha \land X = \hat g^x]
\) (also with a distance bound).
