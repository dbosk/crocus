...
\begin{proof}
  ...
  Now \(\adv\) will provide a correct response with probability~\(p_{\adv}\).
  Hence, \(p_{\S} = p_{\adv}\).
\end{proof}
...
correct responses and \(p_{\S} = 1\) for any successive authentication.
This contradicts that the key is not leaked.
Hence, \(\epsilon < \frac{1}{m}\) or the adversary taints the session or the 
secret leaks.
This proves the following theorem.

\begin{theorem}
  The maximum advantage over random guessing for any \ac{DBTF} adversary is 
  strictly less than \(\frac{1}{m}\) per round.
\end{theorem}
