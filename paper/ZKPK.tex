...
\end{equation}
is \iac{PK} protocol where the prover proves knowledge of \(\alpha, \beta, 
...
%  [m],
%\end{equation*}
%(Signature based on Proofs of Knowledge) which yields a signature \(\sigma\) on 
%\(m\), ensuring that the issuer knows \(\alpha, \beta, \gamma\) such that \(y = 
%g^\alpha h^\beta\) and \(y' = \hat{g}^\gamma\).
