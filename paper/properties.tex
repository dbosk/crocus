...
We have three \emph{malicious} adversaries: Alice, Grace and Mallory.
Alice the activist (and all the protesters) try to increase the count.
Grace the totalitarian dictator tries to decrease the count.
Additionally, Grace tries to deanonymize the participants to arrest them or 
\emph{convince} them to change their minds.
Finally, we have Mallory who is represents another nation state.
Mallory has some interest in affecting the state of Grace's regime, for Mallory's own 
gain, thus supporting either Grace or Alice as he see fits.
Mallory will thus also try to either increase or decrease the count.
...
\begin{requirements}[V]
  \item\label{EligibilityVerif} Eligibility: anyone can verify that each 
    ...
    \item Designated event:%
      \label{DesignatedGracent} prove that the data is designated for the event.
    \end{requirements}
