...
The participant commits to \(\cid\), \ie computes \(c = g^\cid Z^{r_\cid}\), 
where \(r_\cid\rgets \ZZ_q\) is chosen uniformly randomly.
...
\begin{multline*}
  ...
  \PPK\left\{(k, r, r', \cid, r_\cid) : \right. \\
    v^{r'} = v_x v_{xy}^k v_{xy}^{\prime r} \land \pid = g_T^{1/(k+\cid)} \land 
    \\
...
Now both the participant and the witness have the tuple \(\psh = (\pid, \wid, 
