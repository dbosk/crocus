...
following \ac{ZKPK} protocol: \(\PK[k,r][M = g^k Z^r]\).
...
The witness simply should simply get a time-correlated random value from the 
time-stamping service, \(t_s'\gets \TSget\).
...
\(r_1, r_2\gets \ZZ_q^*\) and let \(
  \hat\sigma = (\hat A = A^{r_1},
                \hat B = B^{r_1},
                \hat Z_B = Z_B^{r_1},
                \hat C = C^{r_1 r_2}).
\) (It is not necessary to compute a new \(\hat\sigma\) more than once per 
\(pid\).)
The participant sends \((pid, \hat\sigma)\) to the witness.
The witness verifies that
\begin{align*}
  \hat A &\neq 1 \\
  e(g, \hat B) &= e(Y, \hat A) \\
  e(g, \hat Z_B) &= e(Z, \hat B).
\end{align*}
