...
\begin{figure}
  ...
  \begin{minipage}{\linewidth}
    \begin{align*}
      ...
      W\to S\colon & (\pid, \wid, t_s, t_s', t_e, l, \wsig, \pi_{\wid, 
        \wsig}),\quad \text{where} \\
      ...
      P\to S\colon & (\cid, \pid, \wid, t_s, t_s', t_e, l, \wsig, 
...
If the proof succeeds the witness will compute \(\wid\gets 
  \ACprf[_{\sk_W}][\pid] = g_T^{1/(k_W+\pid)}\) (as above) and in the same way 
...
Now both the participant and the witness have the tuple \(\pi = (\pid, \wid, 
  t_s, t_s', l, \wsig)\) which constitutes a proof share of the participant 
(\cref{fig:ProofFig}).
...
\daniel{Return \(\pi'\), whatever that should be.}
