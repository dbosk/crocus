...
The protester chooses \(d, r\gets \ZZ_q\) uniformly randomly and sets \(M = g^d 
  Z^r\) and send \(M\) to the server.
The protester acts as prover and the \ac{CA} as verifier while they perform the 
following \ac{ZKPK} protocol: \(\PK[d,r][M = g^d Z^r]\).
If the proof succeeds, the \ac{CA} chooses \(a\gets \ZZ_q^*\) and sets \(A = 
  g^a\).
Then it forms the signature \(s = (A, B = A^y, Z_B = B^z = Z^{ay}, C = A^x 
...
\begin{align*}
  ...
  e(g, B) &= e(Y, A) \\
  e(g, Z_B) &= e(Z, B) \\
  e(g, C) &= e(X, A) e(X, B)^d e(X, Z_B)^r.
\end{align*}
Upon success, the protester uses \(sk = (s, d, r)\).
