...
\begin{multline*}
  \mleftright
  \PPK\left\{(k, r, r', \cid, r_{\cid}) : \right. \\
    v^{r'} = v_x v_{xy}^{k_P} v_{xy}^{\prime r} \land \pid = g_T^{1/(k_P+\cid)} 
    \land \\
    \left. c = g^{\cid} Z^{r_{\cid}}
  \right\}.
\end{multline*}
in which \(r' = 1/r_2\).
We can rewrite \(\pid = g_T^{1/(k+\cid)}\) as \(\pid^k = g_T g_T^{-\cid}\).
Afterwards, the \ac{PK} can be designed as follows.
(Remember that this \ac{PK} should be run as \iac{PPK} and repeated multiple 
...
As a consequence, both the protester and the witness have the tuple \(\psh = 
  (\pid, \wid, t_s, t_s', l)\), which forms a proof share for the protester 
(\cref{fig:ProofFig}).
