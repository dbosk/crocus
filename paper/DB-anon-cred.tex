...
In the original \ac{DB} paper by \citet{distance-bounding}, one of the distance-bounding protocol is based on the Schnorr identification scheme~\cite{Schnorr}.
...
\Iac{DBDF} prover must wait for the challenge bit \(b_i\) before responding 
with \(r_i\).
...
%The prover will only provide one of the responses and \(l\) bits of the other. 
%Even if the adversary re-runs the protocol with the same challenges, the 
%randomly chosen \(\rho\) will have changed --- which thus yields another \(R\) 
...
\daniel{I prefer to prove the building blocks separately, then prove the full 
  composite scheme in the analysis section.
  This protocol with a proof would be a contribution in itself.}
