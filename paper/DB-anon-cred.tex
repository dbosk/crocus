...
To achieve the \ac{DB} properties we must run this protocol more rounds than if 
we only need to achieve \ac{PK}.
The security is summarized in \cref{DBSchnorrSecurity}.
...
\begin{table}
  \caption{\label{DBSchnorrSecurity}%
    Summary of the security of the \ac{DB} Schnorr protocol for \(n\) rounds.
  }
  \centering
  \begin{tabular}{lr}
    \toprule
    \textbf{Property} & \textbf{Bits of security} \\
    \midrule
    Soundness (\acs{PK} and \acs{DBIF}) & \(k\cdot n\) \\
    \Acl{DBDF} & \((\log_2 q + l)\cdot n\) \\
    \Acl{DBMF} & \(n\) \\
    %\Acf{DBDH} & \(kn\) \\
    \bottomrule
  \end{tabular}
