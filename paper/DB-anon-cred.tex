...
\begin{proof}
  ...
  choose another challenge uniformly randomly.
  ...
  Thus \(\S\) will provide the same help as before.

  When \(S\) has the second response it can compute the secret of the prover 
  and thus authenticate to the verifier with probability \(p_{\S} = 1\) in any 
  successive session.
...
\begin{figure*}
  ...
  \begin{tabular}{p{0.40\linewidth}cp{0.40\linewidth}}
    ...
    % null
    &
    & Accept if \(R = A^{c_i} g^{s_i}\), \(\hat R =  \hat A^{c_i} \hat g^{\hat 
        s_i}\) and \(\Delta t < t_{\max}\).
    \\
