...
However, ProProx provides \iac{DBPK} protocol for quadratic residues (\ie for 
proving \(\DBPK[\alpha][a = \alpha^2]\)) while in our context, we need 
\iac{DBPK} protocol for discrete logarithms.
...
\ac{DBDH} and \ac{DBIF} resistance --- but not \ac{DBTF} resistance.
...
which is secure against \ac{DBMF}, \ac{DBDF}, \ac{DBDH} and an impersonating 
verifier (\ie public-key).
...
\subsection{Related \acs{DB} protocols}
