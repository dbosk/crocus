...
challenges ensures \ac{DBMF} resistance.
...
The adversary could guess the challenge chosen, \(i\), request \(s_i\) from the 
prover and guess \(s_j\) for \(i\neq j\).
The success probability of this is \(\frac{1}{k} + \frac{k}{q}\) per round, 
thus \((\frac{q + k}{kq})^n\) in total.
...
\begin{proof}[\acs*{DBDH} resistance]
  The adversary must find a collision of challenges for the honest provers 
...
\subsection{Achieving \acl{DBDF} resistance}
