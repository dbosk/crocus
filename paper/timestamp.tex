\subsection{Time-stamping service -- distributed ledger}%
...
In the case of a blockchain, it would be sufficient for a user to only store both the hash of the block containing her proof and the hash of the proof. \sonja{`` --- even if we no longer remember what our data looked like.'' This was commented out, but the question remains: can we still count?}
