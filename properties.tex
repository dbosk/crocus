...
\begin{frame}<presentation>
  \begin{idea}[V\ref{IndividualVerif} Individual verifiability]
    \begin{itemize}
      \item Proofs are stored publicly (e.g.\ anonymous blackboard).
...
\begin{frame}<presentation>
  \begin{idea}[Created after start, before end]
    \begin{itemize}
      \item This implies a partially ordered set.
      \item Can be provided by blockchains.
      \item \dots or transactional 
        posets~\cite{BlockchainFreeCryptocurrencies}.
    \end{itemize}
  \end{idea}

  \pause

  \begin{idea}[At location]
    \begin{itemize}
      \item Use \iac{LP}.
      \item Witnessed (signed) by other participants.
    \end{itemize}
  \end{idea}
\end{frame}
...
\begin{frame}<presentation>
  ...
  \begin{idea}[P\ref{VotePrivacy} Vote privacy]
    \begin{itemize}
...
\begin{frame}<presentation>
  \begin{idea}[P\ref{ReceiptFreeness} Receipt freeness (deniability)]
    \begin{itemize}
